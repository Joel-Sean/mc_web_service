...
public class StringUtil {
	...
	private static Pattern EMAIL_PATTERN = Pattern.compile("^([a-z0-9A-Z_]+[-|\\.]?)+[a-z0-9A-Z_]@([a-z0-9A-Z]+(-[a-z0-9A-Z]+)?\\.)+[a-zA-Z]{2,}$");
	private static Pattern MOBILE_PATTERN_CN = Pattern.compile("\\d{11}");
	...
	/*
	 * If parameter string lenght is greater than 4, the string will be masked. Otherwise, will return original String.
	 * This function will return string by replacing middle four characters to four '*'. 
	 * 
	 */
	...
	/**
	 * If parameter string length is greater than 4, the string will be masked. Otherwise, will return original String.
	 * This function will return string by keeping the first 2 and last 2 characters, the rest of the chars will be masked with four '*'
	 * @param str
	 * @return
	 */
	...
	public static boolean checkMobileCN(String mobileNo) {
		if (isEmpty(mobileNo)) {
			return false;
		}
		Matcher matcher = MOBILE_PATTERN_CN.matcher(mobileNo);
		return matcher.matches();			
	}
